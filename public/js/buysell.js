$(document).ready(function() {
    ...
    //Buysell table
    buysell_table = $('#buysell_table').DataTable({
        processing: true,
        serverSide: true,
        scrollY: "75vh",
        scrollX:        true,
        ...
        ajax: {
            url: '/buysell',
            data: function(d) {
                if ($('#buysell_list_filter_type').length) {
                    d.type_status = $('#buysell_list_filter_type').val();
                }
                if ($('#buysell_list_filter_location_id').length) {
                    d.location_id = $('#buysell_list_filter_location_id').val();
                }
                if ($('#buysell_list_filter_supplier_id').length) {
                    d.supplier_id = $('#buysell_list_filter_supplier_id').val();
                }
                if ($('#buysell_list_filter_payment_status').length) {
                    d.payment_status = $('#buysell_list_filter_payment_status').val();
                }
                if ($('#buysell_list_filter_status').length) {
                    d.status = $('#buysell_list_filter_status').val();
                }

                var start = '';
                var end = '';
                if ($('#buysell_list_filter_date_range').val()) {
                    start = $('input#buysell_list_filter_date_range')
                        .data('daterangepicker')
                        .startDate.format('YYYY-MM-DD');
                    end = $('input#buysell_list_filter_date_range')
                        .data('daterangepicker')
        ...
        fnDrawCallback: function(oSettings) {
            __currency_convert_recursively($('#buysell_table'));
        },
        "footerCallback": function ( row, data, start, end, display ) {
            ...
            for (var r in data){
                ...
                console.log(__currency_trans_from_en(total_purchase))
    ...
    $(document).on(
        'change',
        '#buysell_list_filter_type, \
        #buysell_list_filter_location_id, \
        #buysell_list_filter_supplier_id, \
        #buysell_list_filter_payment_status,\
        #buysell_list_filter_status',
        function() {
            buysell_table.ajax.reload();
        }
    );
