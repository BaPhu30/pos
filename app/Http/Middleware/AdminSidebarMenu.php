                ...
                $menu->dropdown(
                    ...
                    function ($sub) use ($common_settings) {
                        ...
                        if (auth()->user()->can('buysell.view') || auth()->user()->can('view_own_purchase')) {
                            $sub->url(
                                action('BuySellController@index'),
                                __('lang_v1.list_buy_sell_product'),
                                ['icon' => 'fa fas fa-list', 'active' => request()->segment(1) == 'purchases' && request()->segment(2) == null]
                            );
                        }
                        if (auth()->user()->can('buysell.create')) {
                            $sub->url(
                                action('BuySellController@create'),
                                __('lang_v1.add_buy_sell'),
                                ['icon' => 'fa fas fa-plus-circle', 'active' => request()->segment(1) == 'purchases' && request()->segment(2) == 'create']
                            );
                        }
                        // if (auth()->user()->can('purchase.update')) {
                    ...
                    ['icon' => 'fa fas fa-plus-circle', 'id' => 'tour_step6']
                )->order(20);
            }
