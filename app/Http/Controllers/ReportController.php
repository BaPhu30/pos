        ...
        if ($request->ajax()) {
            ...
            $datatable =  Datatables::of($products)
                ...
                ->editColumn('total_sold', function ($row) {
                    ...
                    return '<span data-is_quantity="true" class="total_sold" data-orig-value="' . $total_sold . '">' . $this->transactionUtil->num_product($total_sold, false, null, true) . ' ' .  __("product.product") . '</span> ';
                })
                ->editColumn('total_transfered', function ($row) {
                    $total_transfered = 0;
                    if ($row->total_transfered) {
                        $total_transfered =  $row->total_transfered;
                    }

                    return '<span class="total_transfered" data-orig-value="' . $total_transfered . '">' . $this->transactionUtil->num_product($total_transfered, false, null, true) . ' ' .  __("product.product") . '</span> ';
                })
                
                ->editColumn('total_adjusted', function ($row) {
                    $total_adjusted = 0;
                    if ($row->total_adjusted) {
                        $total_adjusted =  $row->total_adjusted;
                    }
                    return '<span class="total_adjusted" data-orig-value="' . $total_adjusted . '">' . $this->transactionUtil->num_product($total_adjusted, false, null, true) . ' ' .  __("product.product") . '</span> ';
                })
                ->editColumn('type', '@lang("lang_v1." . $type)')
                ->editColumn('stock_price', function ($row) {
                    $html = '<span class="total_stock_price" data-orig-value="' . $row->stock_price . '">' . $this->transactionUtil->num_f($row->stock_price, true) . '</span>';
                    return $html;
                })
                ->editColumn('stock_value_by_sale_price', function ($row) {
                    ...
                    $stock_price = $unit_selling_price;
                ...
                ->editColumn('alert_quantity', function ($row) {
                    ...
                    return $this->transactionUtil->num_product($alert_quantity, false, null, true) . ' ' .  __("product.product");
                })
                // Warn if out of stock or stock quantity equal to or less than specified alert quantity
                ->setRowClass(function ($row) {
                    return ($row->enable_stock && $row->stock <= $row->alert_quantity) || $row->stock == 0 ? 'bg-danger' : '';
                })
            ...
            $raw_columns  = ['unit_price', 'total_transfered', 'total_sold', 'total_adjusted', 'stock', 'stock_price', 'stock_value_by_sale_price', 'potential_profit'];
        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($query)
                ...
                ->editColumn('current_stock', function ($row) {
                    if ($row->enable_stock) {
                        return '<span data-is_quantity="true" class="display_currency current_stock" data-currency_symbol=false data-orig-value="' . $row->current_stock . '" data-unit="' . $row->unit . '" >' . (float) $row->current_stock . '</span> ' .$row->unit;
                    } else {
                        return '';
                    }
                })
